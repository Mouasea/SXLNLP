...
class DialogSystem:
    ...
    def load(self):
        ...
        self.load_scenario("scenario/scenario.json")
    ...
    def policy(self, memory):
        ...
        if memory["need_slot"] is None:
            ...
            #在开放的子节点中，添加节点node0
            memory["available_nodes"].append("scenario-node0")
            #执行动作
    ...
    def nlg(self, memory):
        ...
        if memory["action"] == "answer":
            ...
            memory["history"] = memory["bot_response"]
            ...
            # 将本次回答记录到历史回答中
            memory["history"]= memory["bot_response"]
        elif memory["action"] == "Again":
            memory["bot_response"] = '之前的内容是:' + memory["history"]
        return memory

    def generate_response(self, user_input, memory):
        memory["user_input"] = user_input
        memory = self.nlu(memory)
        if memory["hit_intent"] == "scenario-node0":
            memory["action"] = 'Again'
...
if __name__ == '__main__':
    ...
    print(ds.slot_info)
    memory = {"available_nodes":["scenario-node1","scenario-node0"]}
    while True:
